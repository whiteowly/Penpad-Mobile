...
const Main = () => {
  ...
  const handleSetTodoReminder = async (todo: TodoItem, date: Date | null) => {
    ...
    try {
      ...
      if (date) {
        const notifId = await scheduleNotification('Task reminder', todo.text ?? 'Reminder', date);
  ...
  const handleSetSubtaskReminder = async (todoId: string, sub: SubtaskItem, date: Date | null) => {
    ...
    try {
      ...
      if (date) {
        const notifId = await scheduleNotification('Subtask reminder', sub.text ?? 'Reminder', date);
      ...
      <Box className="flex-1 px-6" style={{ backgroundColor }}>
        ...
        <Box className="mt-3">
          ...
          <DateTimePickerModal
            ...
            // Android: use native picker inside modal but avoid system dialog issues
            // Use isDark to style if the library supports it; otherwise styling is limited.
          />
        </Box>
        ...
        <Modal isOpen={showModal} onClose={() => setShowModal(false)} size="lg">
          ...
          <ModalContent>
            ...
            <ModalBody className="mb-4 w-full">
              ...
              onPress={() => setPickerState({ type: 'todo', todoId: todo.id, initialDate: (todo as any).reminderAt ? new Date((todo as any).reminderAt) : new Date() })} 
