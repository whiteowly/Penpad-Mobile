...
const Main = () => {
    ...
    // subscribe to per-user weeklyTodos collection (separate table from main todos)
    useEffect(() => {
        if (!activeUserId) {
            setTodos([]);
            return;
        ...
        const col = collection(db, 'users', activeUserId, 'weeklyTodos');
        ...
        const unsub = onSnapshot(
            ...
            (err) => handleLoadError(err, 'Could not load weekly tasks')
        );
    ...
    const handleAddTodo = async () => {
        ...
        try {
            setIsSaving(true);
            const todoRef = await addDoc(collection(db, 'users', activeUserId, 'weeklyTodos'), {
                text: trimmedValue,
                completed: false,
            ...
            for (const s of newSubtasks) {
                const t = s.trim();
                if (!t) continue;
                await addDoc(collection(db, 'users', activeUserId, 'weeklyTodos', todoRef.id, 'subtasks'), {
                    text: t,
                    completed: false,
                    createdAt: serverTimestamp(),
                });
    ...
    const handleToggleTodo = async (todo: TodoItem) => {
        ...
        try {
            const todoRef = doc(db, 'users', activeUserId, 'weeklyTodos', todo.id);
    ...
    const handleDeleteSubtask = async (todoId: string, subId: string) => {
        ...
        try {
            setDeletingId(subId);
            const subRef = doc(db, 'users', activeUserId, 'weeklyTodos', todoId, 'subtasks', subId);
    ...
    const toggleExpand = (todoId: string) => {
        ...
        const subCol = collection(db, 'users', activeUserId, 'weeklyTodos', todoId, 'subtasks');
    ...
    const handleAddSubtask = async (todoId: string, text: string) => {
        ...
        if (!t) return;
        try {
            await addDoc(collection(db, 'users', activeUserId, 'weeklyTodos', todoId, 'subtasks'), {
                text: t,
                completed: false,
                createdAt: serverTimestamp(),
            });
    ...
    const handleToggleSubtask = async (todoId: string, sub: SubtaskItem) => {
        ...
        try {
            const subRef = doc(db, 'users', activeUserId, 'weeklyTodos', todoId, 'subtasks', sub.id);
